...
public class PostsController {
    ...
    public ResponseEntity<List<PostsTemplateDTO>> getAllPostsForAUser(@PathVariable String createdBy){
        return ResponseEntity.ok().body(postsService.prepareUserPosts(createdBy));
    }

    @GetMapping("/{postId}")
    public ResponseEntity<PostsDTO> getPostById(@PathVariable String postId) throws PostNotFoundException {
        return ResponseEntity.ok().body(postsService.getPostById(postId));
    }

    @GetMapping("/search/tag")
    public ResponseEntity<List<PostsDTO>> searchByTag(@RequestBody PostSearchDTO postSearchDTO){
        return ResponseEntity.ok().body(postsService.getPostByTag(postSearchDTO.getTag()));
    }

    @GetMapping("/search/keyword")
    public ResponseEntity<List<PostsDTO>> searchByKeyword(@RequestBody PostSearchDTO postSearchDTO){
        return ResponseEntity.ok().body(postsService.getPostByKeyword(postSearchDTO.getKeyword()));
    }

    @PostMapping("/new")
    public ResponseEntity<PostsDTO> postQuestion(@RequestBody PostsDTO postsDTO){
        return ResponseEntity.status(HttpStatus.CREATED).body(postsService.saveNewPost(postsDTO));
    }

    @PutMapping("/post")
    public ResponseEntity<PostsDTO> putQuestion(@RequestBody PostsDTO postsDTO) throws PostNotFoundException {
        return ResponseEntity.status(HttpStatus.ACCEPTED).body(postsService.updatePost(postsDTO));
    }
    ...
    public ResponseEntity<PostsDTO> reactToPost(@RequestBody PostReactionDTO postReactionDTO) throws PostNotFoundException {
        return ResponseEntity.status(HttpStatus.ACCEPTED).body(postsService.reactToPost(postReactionDTO));
    }
